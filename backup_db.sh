...
DB_PATH="/home/quinton/projects/learningTech/dev.db"

mkdir -p "$BACKUP_DIR"

if [ -f "$DB_PATH" ]; then
    sqlite3 "$DB_PATH" ".backup $BACKUP_DIR/db_backup_$TIMESTAMP.db" 2>/dev/null || cp "$DB_PATH" "$BACKUP_DIR/db_backup_$TIMESTAMP.db"
